...
E-ink display screen for managing to-do tasks.
"""
...
class ToDoScreen:
    ...
    def _save_todos(self):
        ...
        os.makedirs(os.path.dirname(self.todos_file) if os.path.dirname(self.todos_file) else '.', exist_ok=True)
    ...
    def delete_todo(self):
        ...
        if 0 <= self.current_index < len(self.todos):
            deleted_text = self.todos[self.current_index]['text']
            del self.todos[self.current_index]
            self._save_todos()
            # Adjust current index if needed
            if self.current_index >= len(self.todos) and self.current_index > 0:
                self.current_index -= 1
            self.logger.info(f"Deleted todo: {deleted_text}")
    ...
    def move_down(self):
        """Move selection down"""
        if self.current_index < len(self.todos) - 1:
            self.current_index += 1
            # Check if we need to change pages
    ...
    def render(self) -> Image.Image:
        ...
        """
        # Reload todos to get latest from web interface
        self._load_todos()
        ...
        if not self.todos:
            message = "No tasks yet. Add one from the web interface!"
            message_bbox = draw.textbbox((0, 0), message, font=self.item_font)
            message_width = message_bbox[2] - message_bbox[0]
            draw.text(
                ((self.width - message_width) // 2, self.height // 2),
                message,
                fill='gray',
                font=self.item_font
            )
            ...
            if total_pages > 1:
                ...
                draw.text(
                    ((self.width - page_width) // 2, self.height - 60),
                    page_info,
                    fill='gray',
                    font=self.item_font
                )
